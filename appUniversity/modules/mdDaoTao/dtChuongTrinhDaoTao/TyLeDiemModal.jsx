...
import { handleTyLeDiem } from './redux';
...
class TyLeDiemModal extends AdminModal {
    ...
    onSubmit = () => {
        ...
        this.props.handleTyLeDiem({ items: this.props.listMonCtdtChon, maKhungDaoTao: this.props.ma, tyLeDiem: T.stringify(listDiem.map(i => ({ loaiThanhPhan: i.loaiThanhPhan, loaiLamTron: i.loaiLamTron, phanTram: i.phanTram }))) }, () => {
            this.hide();
            this.props.handleSave(() => T.alert('Cập nhật thành công!', 'success', false, 500));
        });
    }
        ...
        if (value) {
            ...
            if (value.selected) {
                let data = {
                    ...
                    phanTram: '',
                    loaiLamTron: 0.5,
                    phanTramMin: value.id == 'CK' ? 50 : 0,
                    phanTramMax: value.id != 'CK' ? 50 : 100,
                };
                listDiem.push(data);
...
const mapActionsToProps = { handleTyLeDiem };
