...
class ImportPage extends AdminPage {
    ...
    componentDidMount() {
        T.socket.on('import-chung-chi', ({ status, items, falseItems }) => {
    ...
    downloadExcel = () => {
        T.handleDownload('/api/dt/chung-chi-sinh-vien/import/download-template');
    }
    ...
    table = (list, className) => renderTable({
        ...
        renderHead: () => (<tr>
            ...
            <th style={{ width: 'auto', whiteSpace: 'nowrap' }}>Trình độ</th>
            ...
            <th style={{ width: 'auto', whiteSpace: 'nowrap' }}>Điểm kỹ năng</th>
        ...
        renderRow: (item, index) => {
            return (
                <tr key={`${index}${className}`}>
                    ...
                    <TableCell style={{ whiteSpace: 'nowrap' }} content={item.diem && typeof item.diem == 'object' ? Object.keys(item.diem).map(i => <div key={i}>{this.mapperSkill[i]}: {item.diem[i]}</div>) : ''} />
                    <TableCell style={{ whiteSpace: 'nowrap' }} content={item.score} />
        ...
        return this.renderPage({
            ...
            content: <div className='tile'>
                <div className='rows'>
                    ...
                    {
                        !status ? <FileBox postUrl='/user/upload' uploadType='ImportChungChi' userData='ImportChungChi'
            ...
            backRoute: '/user/dao-tao/student-certificate-management',
        });
    }
}
