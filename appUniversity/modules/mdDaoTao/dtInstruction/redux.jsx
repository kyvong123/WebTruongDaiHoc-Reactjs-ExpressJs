...
// Reducer ------------------------------------------------------------------------------------------------------------
export default function dtInstructionReducer(state = null, data) {
    switch (data.type) {
        default:
            return state;
    }
...
// Actions ----------------------------------------------------------------------------------------------------
T.initPage('dtInstructionPage');
export function getInstructionPage(pageNumber, pageSize, done) {
    const page = T.updatePage('dtInstructionPage', pageNumber, pageSize);
    return () => {
        const url = `/api/dt/instruction/page/${page.pageNumber}/${page.pageSize}`;
        T.get(url, data => {
            if (data.error) {
...
export function getInstruction(id, done) {
    return () => {
        const url = `/api/dt/instruction/${id}`;
        T.get(url, res => {
            if (res.error) {
