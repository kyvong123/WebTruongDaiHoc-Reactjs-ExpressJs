...
import { getAllDtDmTinhTrangHocPhan, createDtDmTinhTrangHocPhan, updateDtDmTinhTrangHocPhan, deleteDtDmTinhTrangHocPhan } from './redux';
...
class DtDmTinhTrangHocPhanPage extends AdminPage {
    componentDidMount() {
        T.ready('/user/dao-tao', () => {
            this.props.getAllDtDmTinhTrangHocPhan();
        });
    }
    ...
    delete = (e, item) => {
        ...
        T.confirm('Xóa Thứ', `Bạn có chắc bạn muốn xóa Thứ ${item.ten ? `<b>${item.ten}</b>` : 'này'} ?`, true, isConfirm => {
            isConfirm && this.props.deleteDtDmTinhTrangHocPhan(item.ma);
        });
    }

    render() {
        const permission = this.getUserPermission('dtDmTinhTrangHocPhan');
        let list = this.props.dtDmTinhTrangHocPhan && this.props.dtDmTinhTrangHocPhan.items ? this.props.dtDmTinhTrangHocPhan.items : null;
        ...
        return this.renderPage({
            ...
            breadcrumb: [
                ...
                <Link key={0} to='/user/dao-tao/data-dictionary'>Từ điển dữ liệu</Link>,
                'Tình trạng học phần'
            ],
            content: <>
                <div className='tile'>{table}</div>
                <EditModal ref={e => this.modal = e} readOnly={!permission.write} create={this.props.createDtDmTinhTrangHocPhan} update={this.props.updateDtDmTinhTrangHocPhan} />
            </>,
...
const mapStateToProps = state => ({ system: state.system, dtDmTinhTrangHocPhan: state.daoTao.dtDmTinhTrangHocPhan });
const mapActionsToProps = { getAllDtDmTinhTrangHocPhan, createDtDmTinhTrangHocPhan, updateDtDmTinhTrangHocPhan, deleteDtDmTinhTrangHocPhan };
export default connect(mapStateToProps, mapActionsToProps)(DtDmTinhTrangHocPhanPage);