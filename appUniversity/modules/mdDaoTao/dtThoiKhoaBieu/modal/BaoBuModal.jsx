...
import { DtTKBCustomBaoBuCreate, DtTKBCustomBaoBuUpdate } from 'modules/mdDaoTao/dtThoiKhoaBieu/redux';
...
class BaoBuModal extends AdminModal {
    ...
    onShow = (item) => {
        let { ngayHoc, thu, tietBatDau, phong, soTietBuoi, coSo, shccGiangVien, shccTroGiang, isEdit } = item;
        if (isEdit) {
    ...
    checkBu = (item, soTietBuoi) => {
        ...
        let dataBu = dataTuan.filter(i => i.idNgayNghi == item.idTuan),
    ...
    checkTrung = (data) => {
        const { soTietBuoi, tietBatDau, ngayBu } = data;
        return this.props.dataTuan.filter(i => !i.isNgayLe && !i.isNghi).find(item => {
    ...
    onSubmit = (e) => {
        e && e.preventDefault();
        let { dataBu, isEdit, item, coSo } = this.state, { dataTiet } = this.props;
        ...
        if (isEdit) {
            ...
            this.setState({ isWait: true });
            this.props.DtTKBCustomBaoBuCreate(data, item, (result) => {
                this.hide();
                this.props.baoBu(result);
            });
        }
    }
    ...
    render = () => {
        let { isChooseNgay, isChooseTiet, isChooseSoTiet, dataBu, isEdit, coSo, isWait, isMail, item } = this.state,
            { tenMonHoc, maHocPhan } = item || {},
            isShow = isChooseNgay && isChooseTiet && isChooseSoTiet;
        return this.renderModal({
            title: 'Lịch báo bù học phần',
            size: 'large',
            isShowSubmit: isShow,
            isLoading: isWait,
...
const mapActionsToProps = { DtTKBCustomBaoBuCreate, DtTKBCustomBaoBuUpdate };
