...
import { createDangKyHocPhan, updateDangKyHocPhan } from './redux';
...
class HocPhanModal extends AdminModal {
    ...
    checkTrung = (item) => {
        let fullData = this.state.listHocPhan.filter(hocPhan => hocPhan.maHocPhan == item.maHocPhan);
        fullData.forEach(hocPhan => {
            let { ngayBatDau: iNgayBatDau, ngayKetThuc: iNgayKetThuc, soTietBuoi: iSoTietBuoi, thu: iThu, tietBatDau: iTietBatDau } = hocPhan;
            this.props.hocPhan?.dataKetQua.forEach(ketQuaDangKy => {
                if (ketQuaDangKy.maMonHoc != hocPhan.maMonHoc) {
                    const { thu, tietBatDau, soTietBuoi, ngayBatDau, ngayKetThuc } = ketQuaDangKy;

                    let thuBatDau = new Date(iNgayBatDau).getDay() + 1;
                    if (thuBatDau == 1) thuBatDau = 8;

                    if (thuBatDau != iThu) {
                        iNgayBatDau += 7 * DATE_UNIX;
                    }
                    ...
                    if (!(iNgayKetThuc < ngayBatDau || iNgayBatDau > ngayKetThuc)) {
                        ...
                        if (thu == iThu && !(iTietKetThuc < tietBatDau || iTietBatDau > tietKetThuc)) {
                            this.setState({ isTrung: hocPhan.maHocPhan, buoiTrung: hocPhan.buoi }, () => T.notify(`Học phần này bị trùng lịch với học phần ${ketQuaDangKy.maHocPhan}`, 'warning'));
                        }
                    }
                }
            });
    ...
    chonHocPhan = (item) => {
        ...
        if (isHocPhan) {
            ...
            if (item.isOnlyKhoa && item.khoaDangKy != this.props.userData.khoa) {
        ...
        if (hocPhanChon) {
            ...
            if (isUpdate) {
                T.confirm('Xác nhận', `Bạn sẽ đăng ký học phần ${hocPhanChon} thay cho ${currHocPhan} (${T.parse(tenMonHoc, { vi: '' })?.vi})`, 'warning', true, isConfirm => {
                    if (isConfirm) {
                        this.props.loading((done) => {
                            this.props.updateDangKyHocPhan({
                                userData: this.props.userData, currHocPhan, newHocPhan: hocPhanChon, filter
                            }, done);
                            this.hide();
                        });
                    }
                ...
                T.confirm('Xác nhận', `Bạn sẽ đăng ký học phần ${hocPhanChon}: ${T.parse(tenMonHoc, { vi: '' })?.vi}?`, 'warning', true, isConfirm => {
                    ...
                    if (isConfirm) {
                        this.props.loading((done) => {
                            this.props.createDangKyHocPhan({
                                userData: this.props.userData, hocPhan: hocPhanChon, filter
                            }, done);
                            this.hide();
                        });
                    }
        ...
        renderRow: (item) => {
            ...
            if (rowSpan) {
                for (let i = 0; i < rowSpan; i++) {
                    ...
                    let isTrung = this.state.isTrung == hocPhan.maHocPhan && this.state.buoiTrung == hocPhan.buoi;
                    if (i == 0) {
                        rows.push(
                            <tr key={rows.length} style={{ backgroundColor: hocPhan.tinhTrang == 3 ? '#ffcccb' : (isCheck ? (isTrung ? '#f7de97' : '#90EE90') : '#ffffff') }}>
                                ...
                                <TableCell type='date' dateFormat='dd/mm/yyyy' style={{ textAlign: 'center', whiteSpace: 'nowrap' }} content={hocPhan.ngayBatDau} rowSpan={rowSpan} />
                                <TableCell type='date' dateFormat='dd/mm/yyyy' style={{ textAlign: 'center', whiteSpace: 'nowrap' }} content={hocPhan.ngayKetThuc} rowSpan={rowSpan} />
