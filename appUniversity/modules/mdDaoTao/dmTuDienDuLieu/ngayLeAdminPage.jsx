...
class DmNgayLePage extends AdminPage {
    ...
    componentDidMount() {
        ...
        T.ready(this.menu == 'category' ? '/user/category' : '/user/dao-tao', () => {
    ...
    render() {
        ...
        return this.renderPage({
            ...
            breadcrumb: [
                <Link key={0} to={this.menu == 'category' ? '/user/category' : '/user/dao-tao'}>{this.menu == 'category' ? 'Danh mục' : 'Đào tạo'}</Link>,
            ...
            backRoute: this.menu == 'category' ? '/user/category' : '/user/dao-tao/data-dictionary',
