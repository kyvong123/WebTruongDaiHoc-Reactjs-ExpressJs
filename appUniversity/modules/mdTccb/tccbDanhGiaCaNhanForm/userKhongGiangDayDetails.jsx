...
import { getTccbDanhGiaChuyenVienUser, createTccbDanhGiaKhongGiangDayUser, deleteTccbDanhGiaKhongGiangDayUser } from './redux/redux';
...
import { EditDanhGiaModal, EditDiemThuongModal, EditNoiDungModal } from './listUserGiangVienModal';
...
class FormChuyenVienDetails extends AdminPage {
    state = { danhGiaNam: {}, diemThuong: [], diemTru: [], nhiemVuKhac: [], nam: '' }

    componentDidMount() {
        T.ready('/user', () => {
            const route = T.routeMatcher('/user/info/danh-gia-khong-giang-day/:nam').parse(window.location.pathname);
    ...
    load = (done) => {
        this.props.getTccbDanhGiaChuyenVienUser(this.state.nam, item => {
            const { danhGiaNam, diemThuong, diemTru, nhiemVuKhac } = item;
            this.props.getTccbKhungDanhGiaCanBoAll(this.state.nam, khungDanhGiaCanBo => {
                this.setState({ danhGiaNam, diemThuong, diemTru, nhiemVuKhac, khungDanhGiaCanBo }, done);
            });
        });
    }
    ...
    createNoiDung = (item, done) => {
        this.props.createTccbDanhGiaKhongGiangDayUser(item, () => this.load(done));
    }

    deleteNoiDung = (item) => {
        T.confirm('Xóa đăng ký', 'Bạn có chắc muốn xóa đăng ký này?', 'info', isConfirm => {
            isConfirm && this.props.deleteTccbDanhGiaKhongGiangDayUser(item.id, item.nam, () => this.load());
        });
    }
    ...
    render() {
        ...
        let { danhGiaNam, diemThuong, diemTru, nhiemVuKhac, nam, khungDanhGiaCanBo } = this.state;
        ...
        let table = renderTable({
            ...
            renderRow: (item, index) => {
                ...
                if (index == 0) {
                    content = <>
                        {/* Nhiệm vụ 1: Nghiên cứu khoa học */}
                                    ...
                                    <TableCell type='buttons' style={{ textAlign: 'center' }} rowSpan={3}>
                                        ...
                                        {isDangKy ? (
                                            <Tooltip title='Xóa đăng ký' arrow>
                                                <button className='btn btn-danger' onClick={() => this.deleteNoiDung(menu)}>
                        ...
                        <tr>
                            ...
                            {isTuDanhGia && <TableCell style={{ textAlign: 'right' }} content={<b>{`${item.totalTuDanhGiaKhac} điểm`}</b>} />}
        ...
        return this.renderPage({
            ...
            content: <>
                ...
                <EditNoiDungModal ref={e => this.noiDungModal = e} create={this.createNoiDung} isDangKy={isDangKy} isTuDanhGia={isTuDanhGia} nam={nam} />
...
const mapActionsToProps = { getTccbDanhGiaChuyenVienUser, createTccbDanhGiaKhongGiangDayUser, deleteTccbDanhGiaKhongGiangDayUser, updateTccbDanhGiaCaNhanDiemThuongUser, getTccbKhungDanhGiaCanBoAll, createTccbDanhGiaCaNhanDiemThuongUser, deleteTccbDanhGiaCaNhanDiemThuongUser };
