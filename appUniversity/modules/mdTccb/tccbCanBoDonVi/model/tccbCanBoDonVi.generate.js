...
module.exports = app => {
    const db = 'main';
    const tableName = 'TCCB_CAN_BO_DON_VI';
    const type = 'table';
    const schema = {
        TEN: {
            type: 'NVARCHAR2',
    ...
    const methods = {
        'searchPage': 'TCCB_CAN_BO_DON_VI_SEARCH_PAGE'
    };
    app.model.tccbCanBoDonVi = {
        create: (data, done) => new Promise((resolve, reject) => {
            ...
            if (statement.length == 0) {
                ...
                const sql = 'INSERT INTO TCCB_CAN_BO_DON_VI (' + statement.substring(2) + ') VALUES (' + values.substring(2) + ')';
                app.database.oracle.connection.main.executeExtra(sql, parameter, (error, resultSet) => {
                    if (error == null && resultSet && resultSet.lastRowid) {
                        app.model.tccbCanBoDonVi.get({ rowId: resultSet.lastRowid }).then(item => {
                            done && done(null, item);
                            resolve(item);
                        }).catch(error => {
                            done && done(error);
        ...
        get: (condition, selectedColumns, orderBy, done) => new Promise((resolve, reject) => {
            ...
            const sql = 'SELECT ' + app.database.oracle.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT * FROM TCCB_CAN_BO_DON_VI' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '') + ') WHERE ROWNUM=1';
            app.database.oracle.connection.main.executeExtra(sql, parameter, (error, resultSet) => {
                if (error) {
                    done && done(error);
                    reject(error);
        ...
        getAll: (condition, selectedColumns, orderBy, done) => new Promise((resolve, reject) => {
            ...
            condition = app.database.oracle.buildCondition(obj2Db, condition, ' AND ');
            const parameter = condition.parameter ? condition.parameter : {};
            const sql = 'SELECT ' + app.database.oracle.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM TCCB_CAN_BO_DON_VI' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '');
            app.database.oracle.connection.main.executeExtra(sql, parameter, (error, resultSet) => {
                if (error) {
                    done && done(error);
                    reject(error);
        ...
        getPage: (pageNumber, pageSize, condition, selectedColumns, orderBy, done) => new Promise((resolve, reject) => {
            ...
            const sqlCount = 'SELECT COUNT(*) FROM TCCB_CAN_BO_DON_VI' + (condition.statement ? ' WHERE ' + condition.statement : '');
            app.database.oracle.connection.main.executeExtra(sqlCount, parameter, (error, res) => {
                if (error) {
                    ...
                    const sql = 'SELECT ' + app.database.oracle.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT TCCB_CAN_BO_DON_VI.*, ROW_NUMBER() OVER (ORDER BY ' + (orderBy ? orderBy : keys) + ') R FROM TCCB_CAN_BO_DON_VI' + (condition.statement ? ' WHERE ' + condition.statement : '') + ') WHERE R BETWEEN ' + (leftIndex + 1) + ' and ' + (leftIndex + pageSize);
                    app.database.oracle.connection.main.executeExtra(sql, parameter, (error, resultSet) => {
                        if (error) {
                            done && done(error);
                            reject(error);
        ...
        update: (condition, changes, done) => new Promise((resolve, reject) => {
            ...
            if (Object.keys(condition).length == 0) {
                ...
                const sql = 'UPDATE TCCB_CAN_BO_DON_VI SET ' + changes.statement + (condition.statement ? ' WHERE ' + condition.statement : '');
                app.database.oracle.connection.main.executeExtra(sql, parameter, (error, resultSet) => {
                    if (error == null && resultSet && resultSet.lastRowid) {
                        app.model.tccbCanBoDonVi.get({ rowId: resultSet.lastRowid }).then(item => {
                            done && done(null, item);
        ...
        delete: (condition, done) => new Promise((resolve, reject) => {
            ...
            if (Object.keys(condition).length == 0) {
                ...
                const parameter = condition.parameter ? condition.parameter : {};
                const sql = 'DELETE FROM TCCB_CAN_BO_DON_VI' + (condition.statement ? ' WHERE ' + condition.statement : '');
        ...
        count: (condition, done) => new Promise((resolve, reject) => {
            ...
            condition = app.database.oracle.buildCondition(obj2Db, condition, ' AND ');
            const parameter = condition.parameter ? condition.parameter : {};
            const sql = 'SELECT COUNT(*) FROM TCCB_CAN_BO_DON_VI' + (condition.statement ? ' WHERE ' + condition.statement : '');
        ...
        searchPage: (pagenumber, pagesize, filter, searchterm, done) => new Promise((resolve, reject) => {
            app.database.oracle.connection.main.executeExtra('BEGIN :ret:=tccb_can_bo_don_vi_search_page(:pagenumber, :pagesize, :filter, :searchterm, :totalitem, :pagetotal); END;',
