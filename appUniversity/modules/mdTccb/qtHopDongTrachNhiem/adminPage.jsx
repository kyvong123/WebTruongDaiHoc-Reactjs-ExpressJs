...
import { getQtHopDongTrachNhiemPage, updateQtHopDongTrachNhiem, deleteQtHopDongTrachNhiem, createQtHopDongTrachNhiem, downloadWord } from './redux';
...
class QtHopDongTrachNhiemPage extends AdminPage {
    ...
    componentDidMount() {
        T.ready('/user/tccb', () => {
            ...
            T.showSearchBox(() => {
                let filterCookie = T.getCookiePage('pageQtHopDongTrachNhiem', 'F'),
    ...
    changeAdvancedSearch = (isInitial = false, isReset = false) => {
        let { pageNumber, pageSize, pageCondition } = this.props && this.props.qtHopDongTrachNhiem && this.props.qtHopDongTrachNhiem.page ? this.props.qtHopDongTrachNhiem.page : { pageNumber: 1, pageSize: 50, pageCondition: {} };
        ...
        this.setState({ filter: pageFilter }, () => {
            this.getPage(pageNumber, pageSize, pageCondition, (page) => {
                if (isInitial) {
                    ...
                    const filterCookie = T.getCookiePage('pageQtHopDongTrachNhiem', 'F');
    ...
    delete = (e, item) => {
        T.confirm('Xóa hợp đồng', 'Bạn có chắc bạn muốn xóa hợp đồng này?', 'warning', true, (isConfirm) => {
            isConfirm &&
                this.props.deleteQtHopDongTrachNhiem(item.ma, (error) => {
    ...
    render() {
        ...
        let { pageNumber, pageSize, pageTotal, totalItem, pageCondition, list } = this.checked ? (this.props.qtHopDongTrachNhiem && this.props.qtHopDongTrachNhiem.pageGr ? this.props.qtHopDongTrachNhiem.pageGr : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, pageCondition: {}, list }) : this.props.qtHopDongTrachNhiem && this.props.qtHopDongTrachNhiem.page ? this.props.qtHopDongTrachNhiem.page : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, pageCondition: {}, list: [] };
            ...
            table = renderTable({
                ...
                renderRow: (item, index) => (
                    <tr key={index}>
                            ...
                            content={
                                <>
                                    <a href={permission.write ? '/user/tccb/qua-trinh/hop-dong-trach-nhiem/' + item.ma : ''}>
                                        <span>{(item.ho ? item.ho : '') + ' ' + (item.ten ? item.ten : '')}</span>
                                        <br />
                                        <span>{item.shcc}</span>
                                    </a>
                                </>
                                ...
                                content={
                                    <p>
                                        <span style={{ whiteSpace: 'nowrap' }}>
                                            Từ ngày: <span style={{ color: 'blue' }}>{item.hieuLucHopDong ? new Date(item.hieuLucHopDong).ddmmyyyy() : ''}</span>
                                        </span>
                                        <br />
                                        <span style={{ whiteSpace: 'nowrap' }}>
                        ...
                        {!this.checked && (
                            <TableCell type='buttons' style={{ textAlign: 'center' }} content={item} permission={permission} onEdit={() => (permission.write ? () => this.props.history.push(`/user/tccb/qua-trinh/hop-dong-trach-nhiem/${item.ma}`) : T.notify('Vui lòng liên hệ phòng Tổ chức - cán bộ', 'warning'))} onDelete={this.delete}>
                                {permission.export && (
...
const mapActionsToProps = {
    ...
    updateQtHopDongTrachNhiem,
    deleteQtHopDongTrachNhiem,
    createQtHopDongTrachNhiem,
    getDmDonViAll,
};
