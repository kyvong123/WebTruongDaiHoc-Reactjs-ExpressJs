...
import { getDmChuDeBlackboxPage, createDmChuDeBlackbox, updateDmChuDeBlackbox, deleteDmChuDeBlackbox } from './redux';
...
class EditModal extends AdminModal {
    onShow = (item) => {
        let { id, ten, doiTuong, thoiGianXuLy, kichHoat, maDonVi } = item ? item : { id: '', ten: '', doiTuong: '', thoiGianXuLy: '', kichHoat: true, maDonVi: null };
        this.setState({ id });
        this.ten.value(ten);
    ...
    onSubmit = (e) => {
        ...
        try {
            const changes = {
                ...
                maDonVi: Number(getValue(this.maDonVi)),
...
class DmChuDeBlackboxPage extends AdminPage {
    componentDidMount() {
        T.ready('/user/category', () => {
            T.onSearch = (searchText) => this.props.getDmChuDeBlackboxPage(undefined, undefined, searchText || '');
            T.showSearchBox();
            this.props.getDmChuDeBlackboxPage();
        });
    }
    ...
    delete = (e, item) => {
        T.confirm('Xóa chủ đề Blackbox', `Bạn có chắc bạn muốn xóa chủ đề Blackbox ${item.ten ? `<b>${item.ten}</b>` : 'này'}?`, 'warning', true, isConfirm => {
            isConfirm && this.props.deleteDmChuDeBlackbox(item.id);
        });
        e.preventDefault();
    }
    ...
    render() {
        const { pageNumber, pageSize, pageTotal, totalItem, pageCondition, list } = this.props.dmChuDeBlackbox && this.props.dmChuDeBlackbox.page ?
            this.props.dmChuDeBlackbox.page : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, list: null };
        ...
        if (list && list.length > 0) {
            table = renderTable({
                ...
                renderHead: () => (
                    <tr>
                        <th style={{ width: '600px', whiteSpace: 'nowrap' }}>Tên chủ đề Blackbox</th>
                        <th style={{ width: '400px', whiteSpace: 'nowrap' }}>Đối tượng liên hệ</th>
                        <th style={{ width: '400px', whiteSpace: 'nowrap' }}>Đơn vị phụ trách</th>
                ...
                renderRow: (item, index) => {
                    ...
                    return <tr key={index}>
                        ...
                        <TableCell type='checkbox' content={item.kichHoat} permission={{ write: true }} onChanged={value => this.props.updateDmChuDeBlackbox(item.id, { kichHoat: Number(value) })} />
                        <TableCell type='buttons' content={item} permission={{ write: true }} onEdit={() => this.modal.show(item)} onDelete={this.delete} />
                    </tr>;
                }
            });
        }
        ...
        return this.renderPage({
            ...
            content: <>
                ...
                <Pagination style={{ marginLeft: '70px' }} {...{ pageNumber, pageSize, pageTotal, totalItem, pageCondition }}
                    getPage={this.props.getDmChuDeBlackboxPage} />
                <EditModal ref={e => this.modal = e} permission={{ write: true }}
                    create={this.props.createDmChuDeBlackbox} update={this.props.updateDmChuDeBlackbox} />
            </>,
...
const mapActionsToProps = { getDmChuDeBlackboxPage, createDmChuDeBlackbox, updateDmChuDeBlackbox, deleteDmChuDeBlackbox };
