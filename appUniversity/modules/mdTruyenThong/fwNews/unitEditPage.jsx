...
import { getDmDonViFaculty, getDmDonViAll } from 'modules/mdDanhMuc/dmDonVi/redux';
            ...
            if (data.error) {
                ...
                this.props.getDmDonViAll(items => {
                    $(this.DonVi.current).select2({
                        data: [{ id: 0, text: 'Trường Đại học Khoa học Xã hội & Nhân văn' }, ...items.map(item => ({ id: item.ma, text: item.ten }))]
                        ,
...
const mapActionsToProps = { getUnitDraftNews, checkLink, updateUnitDraftNews, createUnitDraftNews, getDmDonViFaculty, getDmDonViAll };
