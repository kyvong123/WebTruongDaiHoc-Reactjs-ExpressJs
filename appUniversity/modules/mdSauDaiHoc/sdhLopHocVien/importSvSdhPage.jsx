...
class ImportSinhVienSdhModal extends AdminModal {
    ...
    // Handle Event ===============================================================================================
    onShow = () => {
        const { ma = '', maLopCha = '' } = this.props.sdhLopHocVien?.currentDataLop || {};
        this.setState({ maLop: ma, maLopCha, items: null, failed: null }, () => this.tabs.tabClick(null, 0));
    }
    ...
    onSubmit = () => {
        T.confirm('Xác nhận thêm học viên', `Bạn có chắc muốn thêm những học viên này vào lớp ${this.state.maLop}?`, isConfirm => {
            if (isConfirm) {
    ...
    componentUpload = () => <div className='row'>
        <div className="col-md-12"><p className='pt-3'>Thêm học viên vào lớp bằng tệp Excel(.xlsx). Tải tệp tin mẫu tại <a href='' onClick={e => e.preventDefault() || T.download('/api/sdh/lop-hoc-vien/template')}>đây</a></p></div>
        <FormFileBox className='col-md-12' ref={e => this.file = e} uploadType='sdhLopImportSinhVienSdh' onSuccess={this.onImportSuccess} postUrl={`/user/upload?maLop=${this.state.maLop}&maLopCha=${this.state.maLopCha || ''}`} />
    </div>
    ...
    render = () => {
        return this.renderModal({
            title: 'Tải lên danh sách học viên',
            body: (
...
export default connect(mapStateToProps, mapActionsToProps, null, { forwardRef: true })(ImportSinhVienSdhModal);