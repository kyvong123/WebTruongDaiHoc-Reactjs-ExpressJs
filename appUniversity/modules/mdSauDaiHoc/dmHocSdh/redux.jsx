...
export function getDmHocSdhPage(pageNumber, pageSize, pageCondition, done) {
    ...
    return (dispatch) => {
        const url = `/api/sdh/bac-sdh/page/${page.pageNumber}/${page.pageSize}`;
...
export function getDmHocSdh(ma, done) {
    return () => {
        const url = `/api/sdh/bac-sdh/item/${ma}`;
        T.get(url, (data) => {
            if (data.error) {
...
export function createDmHocSdhByUpload(item, done) {
    return (dispatch) => {
        const url = '/api/sdh/bac-sdh/createFromFile';
...
export const SelectAdapter_DmHocSdhVer2 = {
    ...
    fetchOne: (id, done) => (getDmHocSdh(id, item => item && done && done({ id: item.ma, text: item.ten, maLop: item.maLop })))(),
};
