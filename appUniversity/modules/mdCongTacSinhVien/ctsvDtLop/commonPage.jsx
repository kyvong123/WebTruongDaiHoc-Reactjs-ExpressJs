...
import { getCtsvLopCommonPage, updateCtsvLop, createCtsvLop } from './redux';
// import MultipleCreateModal from './multipleCreateModal';
import { EditModal } from './adminPage';

class CommonPage extends AdminPage {
    componentDidMount() {
        T.ready('/user/ctsv', () => {
            this.props.getCtsvLopCommonPage(1, 50);
        });
    }

    getPage = (pageN, pageS, pageC, done) => {
        let filter = { ...this.state.filter, sort: this.state?.sortTerm || this.defaultSortTerm };
        this.props.getCtsvLopCommonPage(pageN, pageS, pageC, filter, done);
    }
    ...
    render() {
        ...
        const { pageNumber, pageSize, pageTotal, totalItem, pageCondition, list = [] } = this.props.ctsvLop && this.props.ctsvLop.commonPage ? this.props.ctsvLop.commonPage : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, list: null };
        const table = renderDataTable({
            ...
            renderRow: (item, index) => <tr key={index}>
                ...
                <TableCell permission={permission} type='buttons' onEdit={`/user/ctsv/lop/item?khoa=${item.khoaSinhVien}&heDaoTao=${item.heDaoTao.replace('+', '%2b')}`} />
            </tr>
        });
...
const mapActionsToProps = {
    getCtsvLopCommonPage, createCtsvLop, updateCtsvLop
};
