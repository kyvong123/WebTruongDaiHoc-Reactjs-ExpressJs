// Table name: HCTH_PHAN_HOI { id, canBoGui, key, ngayTao, loai, noiDung, isResolved }
const keys = ['ID'];
const obj2Db = { 'id': 'ID', 'canBoGui': 'CAN_BO_GUI', 'key': 'KEY', 'ngayTao': 'NGAY_TAO', 'loai': 'LOAI', 'noiDung': 'NOI_DUNG', 'isResolved': 'IS_RESOLVED' };

module.exports = app => {
    const db = 'main';
    const tableName = 'HCTH_PHAN_HOI';
    const type = 'table';
    const schema = {
        ID: {
            type: 'NUMBER',
    ...
    const methods = {
        'getAllFrom': 'HCTH_PHAN_HOI_GET_ALL_FROM',
        'getChiDao': 'HCTH_PHAN_HOI_GET_CHI_DAO'
    };
    app.model.hcthPhanHoi = {
        ...
        getChiDao: (key, done) => new Promise((resolve, reject) => {
            app.database.oracle.connection.main.executeExtra('BEGIN :ret:=hcth_phan_hoi_get_chi_dao(:key); END;',
