...
class SignFilePage extends AdminPage {
    ...
    changeAdvancedSearch = (isInitial = false) => {
        let { pageNumber, pageSize } = this.props && this.props.hcthCongVanDen && this.props.hcthCongVanDen.files ? this.props.hcthCongVanDen.files : { pageNumber: 1, pageSize: 50 };
    ...
    checkSelectAll = () => {
        if (this.props.hcthCongVanDen?.files?.list?.find(item => !this.checkBox[item.id]?.value())) {
    ...
    onCheckAll = (value) => {
        if (value) {
            this.props.hcthCongVanDen?.files?.list?.map(item => {
                this.checkBox[item.id]?.value(true);
                this.selected[item.id] = item;
            });
        } else {
            this.props.hcthCongVanDen?.files?.list?.forEach(item => {
    ...
    render() {
        ...
        let { pageNumber, pageSize, pageTotal, totalItem, pageCondition, list } = this.props.hcthCongVanDen && this.props.hcthCongVanDen.files ?
            this.props.hcthCongVanDen.files : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, pageCondition: {}, list: null };
            ...
            renderRow: (item) => {
                return (
                    <tr key={item.R} >
                        ...
                        <TableCell style={{ whiteSpace: 'nowrap' }} content={<Link to={`${baseUrl}/${item.vanBanDenId}`} target='_blank' rel='noreferrer noopener'>{item.soVanBan || 'Chưa có số văn bản'} </Link>} />
