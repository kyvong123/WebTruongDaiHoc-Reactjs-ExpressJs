...
import { getPage, create, update, deleteSignType } from './redux/hcthSignType';
...
class EditModal extends AdminModal {
    ...
    onShow = (item) => {
        let { ma, ten, signLevel, color, phuLuc, height, width, kichHoat } = item || {};
        this.setState({ ma, ten, signLevel, color, phuLuc, height, width, kichHoat }, () => {
            this.ma.value(ma || '');
            this.ten.value(ten || '');
            this.signLevel.value(signLevel || '');
            this.color.value(color || '');
            this.phuLuc.value(phuLuc || '');
    ...
    onSubmit = (e) => {
        try {
            e.preventDefault();
            const data = {};
            ['ma', 'ten', 'signLevel', 'color', 'phuLuc', 'height', 'width', 'kichHoat'].forEach(key => {
    ...
    render = () => {
        ...
        return this.renderModal({
            ...
            body: <div className='row'>
                ...
                <FormTextBox className='col-md-4' ref={e => this.signLevel = e} label='Cấp' readOnly={readOnly} />
                <FormTextBox className='col-md-4' ref={e => this.color = e} label='Màu' readOnly={readOnly} />
                <FormTextBox className='col-md-4' ref={e => this.phuLuc = e} label='Phụ lục' readOnly={readOnly} />
                <FormTextBox className='col-md-4' ref={e => this.height = e} label='Chiều cao' readOnly={readOnly} />
                <FormTextBox className='col-md-4' ref={e => this.width = e} label='Chiều rộng' readOnly={readOnly} />
...
class HcthSignType extends AdminPage {
    state = { filter: {} }
    componentDidMount() {
        T.ready('/user/hcth', () => {
    ...
    delete = (e, item) => {
        e.preventDefault();
        //TODO
        T.confirm('Xóa loại chữ ký', 'Xác nhận xóa loại chữ ký?', 'warning', true, isConfirm =>
...
const mapStateToProps = state => ({ system: state.system, doiTuong: state.hcth.hcthSignType });
const mapActionsToProps = { getPage, create, update, deleteSignType };
export default connect(mapStateToProps, mapActionsToProps)(HcthSignType);