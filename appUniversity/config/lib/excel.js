module.exports = app => {
    ...
    app.excel = {
        ...
        copyWorksheet: (sourceWs, destWs) => {
            destWs.model = sourceWs.model;
            sourceWs.model.merges.forEach(item => destWs.mergeCells(item));
        },

        replaceByData: (worksheet, data) => {
            const placeholderPattern = /\{([^\}]+)\}/g;
            worksheet.eachRow(row => {
                row.eachCell(cell => {
                    if (typeof cell.value === 'string') {
                        const replacedValue = cell.value.replace(placeholderPattern, (match, key) => data[key] || match);
