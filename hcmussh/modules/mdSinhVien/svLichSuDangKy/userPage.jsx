...
import { getDtLichSuDkhpPage } from './redux';
...
class UserPage extends AdminPage {
    ...
    componentDidMount() {
        ...
        this.setState({
            filter: { userMssv: user.mssv }
        });
        T.ready('/user/lich-su-dang-ky', () => {
            T.clearSearchBox();
    ...
    render() {
        ...
        return this.renderPage({
            ...
            icon: 'fa fa-history',
            breadcrumb: ['Lịch sử đăng ký'],
            content: (<>
                <div className='tile'>
                    <div className='row'>
                        <FormSelect ref={(e) => (this.namHoc = e)} className='col-md-6' label='Năm học' data={this.state.namHoc}
                            onChange={(value) => this.setState({ filter: { ...this.state.filter, namHoc: value?.id || '' } })} allowClear
                        />
                        <FormSelect ref={(e) => (this.hocKy = e)} className='col-md-6' label='Học kỳ' data={SelectAdapter_HocKy}
                ...
                <Pagination style={{ marginLeft: '20px' }} {...{ pageNumber, pageSize, pageTotal, totalItem }}
...
const mapStateToProps = state => ({ system: state.system, dtLichSuDkhp: state.student.dtLichSuDkhp });
