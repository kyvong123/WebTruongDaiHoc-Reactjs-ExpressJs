...
export class SubDetail extends AdminModal {
    onShow = (item, listDetail) => {
        this.formTab.tabClick(null, 0);
        this.props.getSubDetailHocPhi(item.idDotDong, result => {
            this.setState({ hocPhiSubDetail: result.subDetail, khoaSinhVien: result.khoaSinhVien });
        });
        this.setState({ item, listDetail });
    }
    tableSubDetail = (data) => {
                        ...
                        tabs={[
                            {
                                id: 0, title: 'Chi tiết', component: < >{this.tableSubDetail(hocPhiSubDetail)}
                                    <div style={{ textAlign: 'right' }}>
                                        <div>Tổng học phí: <b>{T.numberDisplay(Number(this.state.listDetail?.find(item => item.isHocPhi == 1)?.soTien)) || ''} VNĐ </b></div>
                                    </div>
                                </>
                            },
