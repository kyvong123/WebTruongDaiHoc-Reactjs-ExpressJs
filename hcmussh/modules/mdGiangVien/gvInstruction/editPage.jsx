...
class userEditPage extends AdminPage {
    ...
    componentDidMount() {
        T.ready('/user/affair', () => {
            const params = T.routeMatcher('/user/affair/instruction/:id').parse(window.location.pathname);
            this.setState({
    ...
    saveContent = () => {
        ...
        if (!changes.tieuDe) {
            ...
            if (this.state.id) {
                ...
                this.props.createInstruction(changes, () => this.props.history.push('/user/affair/instruction'));
            }
        }
    }
        ...
        return <>
            <div className='tile'>
                ...
                <div className='tile-body'>
                    ...
                    {!readOnly && <FormEditor ref={e => this.noiDungInstruction = e} height={400} label='Nội dung' dangerouslySetInnerHTML={{ _html: this.state.noiDung }} readOnly={readOnly} uploadUrl='/user/upload?category=gvInstruction' />}
                </div>
    ...
    render = () => {
        return this.renderPage({
            ...
            backRoute: '/user/affair/instruction'
        });
    }
}
