...
class HopDongLaoDong extends AdminPage {
    ...
    changeAdvancedSearch = (isInitial = false, isReset = false) => {
        let { pageNumber, pageSize, pageCondition } = this.props && this.props.tccbHopDongLaoDong && this.props.tccbHopDongLaoDong.page ? this.props.tccbHopDongLaoDong.page : { pageNumber: 1, pageSize: 50, pageCondition: {} };
    ...
    render() {
        ...
        const permission = this.getUserPermission('tccbHopDongLaoDong', ['read', 'write', 'delete', 'manage', 'export']);
        let { pageNumber, pageSize, pageTotal, totalItem, pageCondition, list } = this.props.tccbHopDongLaoDong && this.props.tccbHopDongLaoDong.page ? this.props.tccbHopDongLaoDong.page : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, pageCondition: {}, list: [] };
        let table = renderDataTable({
            ...
            renderRow: (item, index) => (
                <tr key={index}>
                    ...
                    <TableCell type='link' style={{ whiteSpace: 'nowrap' }} content={item.soHopDong} onClick={() => window.open(`/user/tccb/hop-dong-lao-dong/${item.ma}`, '_blank')} />
                    ...
                    <TableCell contentClassName='multiple-lines-4' style={{ whiteSpace: item.phanTramHuongMoi ? 'nowrap' : '' }}
                        content={(item.phanTramHuongMoi && item.phanTramHuongMoi.filter(row => row.phanTram).length) ? <>{item.phanTramHuongMoi.map((phanTramHuong, index) =>
                            <span key={index}>{phanTramHuong.phanTram}%: {phanTramHuong.tuNgay ? T.dateToText(Number(phanTramHuong.tuNgay), 'dd/mm/yyyy') : ''} - {phanTramHuong.denNgay ? T.dateToText(Number(phanTramHuong.tuNgay), 'dd/mm/yyyy') : ''}<br /></span>)}</> : item.phanTramHuong} />
                    ...
                    <TableCell type='buttons' style={{ textAlign: 'center' }} content={item} permission={permission}
                        onEdit={() => (permission.write ? this.props.history.push(`/user/tccb/hop-dong-lao-dong/${item.ma}`) : T.notify('Vui lòng liên hệ phòng Tổ chức - Cán bộ', 'warning'))}
        ...
        return this.renderPage({
            ...
            backRoute: '/user/tccb',
            collapse: [
                { icon: 'fa-plus-square', name: 'Thêm hợp đồng mới', permission: permission.write, type: 'info', onClick: (e) => e.preventDefault() || window.open('/user/tccb/hop-dong-lao-dong/new', '_blank') },
                { icon: 'fa-print', name: 'Export', permission: permission.export, onClick: (e) => e.preventDefault() || T.download(T.url('/api/tccb/hop-dong-lao-dong/download-excel'), 'HDLD.xlsx'), type: 'success' }
            ]
        });
    }
}
...
export default connect(mapStateToProps, mapActionsToProps)(HopDongLaoDong);
