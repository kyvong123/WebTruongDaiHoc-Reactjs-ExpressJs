module.exports = app => {
    app.post('/api/tccb/qua-trinh/ky-yeu', app.permission.check('staff:write'), (req, res) =>
        app.model.qtKyYeu.create(req.body.data, (error, item) => res.send({ error, item })));

    app.put('/api/tccb/qua-trinh/ky-yeu', app.permission.check('staff:write'), (req, res) =>
        app.model.qtKyYeu.update({ id: req.body.id }, req.body.changes, (error, item) => res.send({ error, item })));

    app.delete('/api/tccb/qua-trinh/ky-yeu', app.permission.check('staff:write'), (req, res) =>
        app.model.qtKyYeu.delete({ id: req.body.id }, (error) => res.send(error)));

    app.post('/api/tccb/user/qua-trinh/ky-yeu', app.permission.check('staff:login'), (req, res) => {
    ...
    app.put('/api/tccb/user/qua-trinh/ky-yeu', app.permission.check('staff:login'), (req, res) => {
    ...
    app.delete('/api/tccb/user/qua-trinh/ky-yeu', app.permission.check('staff:login'), (req, res) => {
        if (req.session.user) {
            app.model.qtKyYeu.get({ id: req.body.id }, (error, item) => {
                if (error || item == null) {
                    res.send({ error: 'Not found!' });
