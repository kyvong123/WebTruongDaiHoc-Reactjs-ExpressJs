module.exports = app => {
    app.post('/api/tccb/danh-gia-khong-giang-day-user', app.permission.check('staff:login'), async (req, res) => {
        //Dành cho nghiên cứu viên không tham gia giảng dạy tự đánh giá
        try {
            ...
            if (userNgach == '01.003' || !listNgachNcvKhongGiangDay.includes(userNgach)) {
                throw 'Bạn không phải là nghiên cứu viên không tham gia giảng dạy!';
            }
    ...
    // Trưởng đơn vị
    app.post('/api/tccb/unit/danh-gia-khong-giang-day', app.permission.check('manager:login'), async (req, res) => {
        //Dành cho nghiên cứu viên không tham gia giảng dạy
        try {
            ...
            if (canBo.ngach == '01.003' || !listNgachNcvKhongGiangDay.includes(canBo.ngach)) {
                throw 'Người này không phải là nghiên cứu viên không tham gia giảng dạy!';
            }
    ...
    app.delete('/api/tccb/danh-gia-khong-giang-day-user', app.permission.check('staff:login'), async (req, res) => {
        try {
            const userNgach = req.session.user.ngach;
