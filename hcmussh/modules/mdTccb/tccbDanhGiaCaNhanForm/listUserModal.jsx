...
export class EditNoiDungModal extends AdminModal {
    ...
    onShow = ({ item, idFormChuyenVien, loaiCongViec, isDotXuat }) => {
        ...
        this.setState({ item, idFormChuyenVien, loaiCongViec, isDotXuat });
    };

    onSubmit = (e) => {
        e.preventDefault();
        const { isDangKy, isDangKyDotXuat, isTuDanhGia } = this.props;
        const { item, idFormChuyenVien, loaiCongViec, isDotXuat } = this.state;
        const changes = {};
        ...
        this.props.create({ ...item, ...changes, nam: this.props.nam, idFormChuyenVien, loaiCongViec, isDotXuat }, this.hide);
    };

    render = () => {
        const { isDangKy, isDangKyDotXuat, isTuDanhGia } = this.props;
        const { isDotXuat } = this.state;
        const hasEdit = isDangKy || (isDangKyDotXuat && isDotXuat);
        return this.renderModal({
            title: 'Cập nhật công việc',
            body: <div className='row'>
                <FormTextBox className='col-12' ref={e => this.noiDung = e} label='Nội dung' required readOnly={!hasEdit} />
                <FormRichTextBox className='col-12' ref={e => this.moTa = e} label='Mô tả công việc' rows={5} required readOnly={!hasEdit} />
                <FormDatePicker type='time-mask' ref={e => this.thoiHan = e} className='col-12' label='Thời hạn hoàn thành' readOnly={!hasEdit} />
                <FormTextBox type='number' min={0} step={true} className='col-12' ref={e => this.diemLonNhat = e} label='Điểm tối đa' required readOnly={!hasEdit} />
