...
export function getDmDonViGuiCongVanPage(pageNumber, pageSize, pageCondition, done) {
    ...
    return dispatch => {
        ...
        T.get(url, { condition: page.pageCondition }, data => {
            if (data.error) {
                T.notify('Lấy danh sách đơn vị gửi văn bản bị lỗi' + (data.error.message && (':<br>' + data.error.message)), 'danger');
        ...
        }, (error) => T.notify('Lấy danh sách đơn vị gửi văn bản bị lỗi' + (error.error.message && (':<br>' + error.error.message)), 'danger'));
    };
}
...
export function getDmDonViGuiCv(id, done) {
    return () => {
        ...
        T.get(url, { id }, data => {
            if (data.error) {
                T.notify('Lấy thông tin đơn vị gửi văn bản bị lỗi' + (data.error.message && (':<br>' + data.error.message)), 'danger');
                console.error(`GET: ${url}.`, data.error);
            } else {
                done && done(data.item);
            }
    ...
    return dispatch => {
        ...
        T.post(url, { item }, data => {
            if (data.error) {
                ...
                T.notify('Tạo đơn vị gửi văn bản thành công!', 'success');
                dispatch(getDmDonViGuiCongVanPage());
                done && done(data);
            }
        }, (error) => T.notify('Tạo đơn vị gửi văn bản bị lỗi' + (error.error.message && (':<br>' + error.error.message)), 'danger'));
    };
}
...
export function deleteDmDonViGuiCv(id) {
    return dispatch => {
        ...
        T.delete(url, { id }, data => {
            if (data.error) {
                T.notify('Xóa danh mục đơn vị gửi văn bản bị lỗi' + (data.error.message && (':<br>' + data.error.message)), 'danger');
                console.error(`DELETE: ${url}.`, data.error);
            } else {
                T.alert('Danh mục đơn vị gửi văn bản đã xóa thành công!', 'success', false, 800);
                dispatch(getDmDonViGuiCongVanPage());
            }
        }, (error) => T.notify('Xóa đơn vị gửi văn bản bị lỗi' + (error.error.message && (':<br>' + error.error.message)), 'danger'));
    };
}
    ...
    return dispatch => {
        ...
        T.put(url, { id, changes }, data => {
            if (data.error || changes == null) {
                T.notify('Cập nhật thông đơn vị gửi văn bản bị lỗi' + (data.error.message && (':<br>' + data.error.message)), 'danger');
                console.error(`PUT: ${url}.`, data.error);
                done && done(data.error);
            } else {
                T.notify('Cập nhật thông tin đơn vị gửi văn bản thành công!', 'success');
                done && done(data.item);
                dispatch(getDmDonViGuiCongVanPage());
            }
        }, (error) => T.notify('Cập nhật thông tin đơn vị gửi văn bản bị lỗi' + (error.error.message && (':<br>' + error.error.message)), 'danger'));
    };
}
