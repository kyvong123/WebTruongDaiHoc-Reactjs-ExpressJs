...
export class ComponentDiaDiem extends React.Component {
    ...
    render = () => {
        const { label, className, style, readOnly = false, requiredSoNhaDuong = false, fullDisplay = false, noLabel = false, required = false, maxLengthSoNha, requiredAll = false } = this.props;
                ...
                <span>
                    ...
                    <div className='row'>
                        <FormSelect ref={e => this.dmTinhThanhPho = e} data={ajaxSelectTinhThanhPho} onChange={value => this.changeTinhThanhPho(value)} readOnly={readOnly} className={defaultClassName}
                            label={readOnly || requiredAll ? 'Thành phố, tỉnh' : null} placeholder='Thành phố, tỉnh' required={requiredAll} />
                        <FormSelect ref={e => this.dmQuanHuyen = e} data={maTinhThanhPho ? ajaxSelectQuanHuyen(maTinhThanhPho) : []} onChange={value => this.changeQuanHuyen(value)} readOnly={readOnly}
                            className={defaultClassName} label={readOnly || requiredAll ? 'Quận, huyện' : null} placeholder='Quận, huyện' required={requiredAll} />
                        <FormSelect ref={e => this.dmPhuongXa = e} data={maQuanHuyen ? ajaxSelectPhuongXa(maQuanHuyen) : []} onChange={value => this.changePhuongXa(value)} readOnly={readOnly}
                            className={defaultClassName} label={readOnly || requiredAll ? 'Phường, xã' : null} placeholder='Phường, xã' required={requiredAll} />
