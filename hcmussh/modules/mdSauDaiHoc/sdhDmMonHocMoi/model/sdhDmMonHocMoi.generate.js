// Table name: SDH_DM_MON_HOC_MOI { ma, tenTiengViet, tenTiengAnh, tcLyThuyet, tcThucHanh, khoaSdh, kichHoat, tietLt, tietTh, tongTiet, tienQuyet, tongTc }
const keys = ['MA'];
const obj2Db = { 'ma': 'MA', 'tenTiengViet': 'TEN_TIENG_VIET', 'tenTiengAnh': 'TEN_TIENG_ANH', 'tcLyThuyet': 'TC_LY_THUYET', 'tcThucHanh': 'TC_THUC_HANH', 'khoaSdh': 'KHOA_SDH', 'kichHoat': 'KICH_HOAT', 'tietLt': 'TIET_LT', 'tietTh': 'TIET_TH', 'tongTiet': 'TONG_TIET', 'tienQuyet': 'TIEN_QUYET', 'tongTc': 'TONG_TC' };

module.exports = app => {
    const db = 'main';
    const tableName = 'SDH_DM_MON_HOC_MOI';
    const type = 'table';
    const schema = {
        MA: {
            type: 'NVARCHAR2',
    ...
    const methods = {
        'searchPage': 'SDH_DM_MON_HOC_MOI_SEARCH_PAGE'
    };
    app.model.sdhDmMonHocMoi = {
        ...
        searchPage: (pagenumber, pagesize, filter, searchterm, done) => new Promise((resolve, reject) => {
            app.database.oracle.connection.main.executeExtra('BEGIN :ret:=sdh_dm_mon_hoc_moi_search_page(:pagenumber, :pagesize, :filter, :searchterm, :totalitem, :pagetotal); END;',
