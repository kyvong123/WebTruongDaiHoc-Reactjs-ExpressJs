...
import { getDtLichSuDkhpDashBoard, hoanTacDtLichSuDkhpChuyen } from 'modules/mdDaoTao/dtLichSuDkhp/redux';
import { connect } from 'react-redux';
import Pagination from 'view/component/Pagination';
import { Tooltip } from '@mui/material';
class LichSuChuyen extends AdminPage {
    state = {
    ...
    getLichSu = (done) => {
        this.setState({ filter: { ...this.props.filter, thaoTac: 'C' } }, () => {
    ...
    hoanTac = (item) => {
        this.props.hoanTacDtLichSuDkhpChuyen(item);
        console.log(item);
    }
                ...
                return (
                    <tr key={index}>
                        ...
                        <TableCell style={{ whiteSpace: 'nowrap', textAlign: 'center' }} content={
                            <div style={{ color: 'blue' }}><i className='fa fa-lg fa-repeat' /> Chuyển lớp </div>
                        } />
                        <TableCell style={{ whiteSpace: 'nowrap', textAlign: 'left' }} content={
                            item.thaoTac == 'C' ? `Sinh viên được chuyển từ lớp học phần ${item.ghiChu}` : item.ghiChu
                        } />
                        <TableCell type='buttons' style={{ textAlign: 'center' }} content={item}>
                            {item.thaoTac == 'C' ?
                                <Tooltip title='Hoàn tác' arrow>
                                    <button className='btn btn-info' onClick={e => e.preventDefault() || this.hoanTac(item)} >
...
const mapActionsToProps = {
    getDtLichSuDkhpDashBoard, hoanTacDtLichSuDkhpChuyen
};
export default connect(null, mapActionsToProps, null, { forwardRef: true })(LichSuChuyen);