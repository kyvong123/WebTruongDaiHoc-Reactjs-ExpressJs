...
class ImportSinhVienModal extends AdminModal {
    ...
    // Handle Event ===============================================================================================
    onShow = () => {
        const { maLop = '', maLopCha = '' } = this.props.dtLop?.currentDataLop || {};
    ...
    componentUpload = () => <div className='row'>
        <div className="col-md-12"><p className='pt-3'>Thêm sinh viên vào lớp bằng tệp Excel(.xlsx). Tải tệp tin mẫu tại <a href='' onClick={e => e.preventDefault() || T.download('/api/dt/lop-sinh-vien/template')}>đây</a></p></div>
        <FormFileBox className='col-md-12' ref={e => this.file = e} uploadType='dtLopImportSinhVien' onSuccess={this.onImportSuccess} postUrl={`/user/upload?maLop=${this.state.maLop}&maLopCha=${this.state.maLopCha || ''}`} />
    </div>
