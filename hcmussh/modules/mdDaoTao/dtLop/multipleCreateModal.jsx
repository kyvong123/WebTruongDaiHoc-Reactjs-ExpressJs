...
import { getDtNganhPage } from '../dtNganhDaoTao/redux';
import { getDtLopPage, createMultipleDtLop, createDtLopMultiple } from './redux';

class MultipleCreateModal extends AdminModal {
    state = { list: [], dsChuyenNganh: [], khoaSinhVien: '' }
    maLop = {}
    tenLop = {}
    namHocBatDau = {}
    componentDidMount() {
    ...
    getData = (value) => {
        ...
        this.props.getDtNganhPage(1, 1000, '', {}, data => {
            this.setState({
                ...
                this.props.getDtLopPage(1, 1000, '', { khoaSinhVien, heDaoTao: maHe }, page => {
    ...
    onComplete = (e) => {
        ...
        this.props.createMultipleDtLop(listNganh, listChuyenNganh, config, result => {
    ...
    onSubmit = () => {
        T.confirm('Lưu ý', 'Bạn có chắc chắn muốn tạo các lớp này?', 'danger', true, isConfirm => {
            isConfirm && this.props.createDtLopMultiple(this.state.result, this.hide);
        });
    }
    ...
    render = () => {
        return this.renderModal({
            ...
            body: <div className='row align-content-start' style={{ height: '60vh', overflow: 'scroll' }}>
                <FormSelect ref={e => this.khoaSinhVien = e} data={SelectAdapter_DtKhoaDaoTao} className='col-md-6' label='Khoá sinh viên' required onChange={(value) => { this.setState({ khoaSinhVien: value.id }, () => this.heDaoTao.focus()); }} />
                <FormSelect ref={e => this.heDaoTao = e} className='col-md-6' label='Hệ đào tạo' data={SelectAdapter_DmSvLoaiHinhDaoTaoFilter} required disabled={this.state.khoaSinhVien ? false : true} onChange={this.getData} subText />
...
const mapActionsToProps = {
    getDtNganhPage, createMultipleDtLop, getDtLopPage, createDtLopMultiple
};
