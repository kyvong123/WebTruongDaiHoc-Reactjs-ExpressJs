module.exports = app => {
    app.utils = {
        ...
        getUserFullName: (user) => `${user.lastName || ''} ${user.firstName || ''}`.trim().normalizedName(),
        ...
        borderCells: (ws, start = { row: 1, col: 1 }, end = { row: 1, col: 1 }, width = 'thin') => {
            for (let i = start.row; i <= end.row; i++) {
                for (let j = start.col; j <= end.col; j++) {
                    ws.getCell(i, j).border = { top: { style: width }, bottom: { style: width }, right: { style: width }, left: { style: width } };
                }
            }
        }
    };
    ...
    app.notification = {
        send: ({ toEmail = '', title = '', subTitle = '', icon = 'fa-commenting', iconColor = '#1488db', link = '', buttons = [], sendEmail = null, notificationCategory = '', firebaseNotify = false }) => new Promise((resolve, reject) => {
            // Validate data
            ...
            const newNotification = { email: toEmail, notificationCategory, title, subTitle, icon, iconColor, targetLink: link, buttonLink: buttons, sendTime: new Date().getTime() };
            app.model.fwNotification.create(newNotification, (error, item) => {
                if (error || !item) {
                    ...
                    if (firebaseNotify) {
                        app.model.fwUserDeviceToken.get({ email: toEmail }, (errorGetToken, item) => {
                            if (errorGetToken || !item) reject(errorGetToken || `Email ${toEmail} has no token-device`);
                            else {
                                app.firebase.sendNotification({ title, body: subTitle, token: item.deviceToken });
                            }
                        });
                    }
                    if (sendEmail) {
