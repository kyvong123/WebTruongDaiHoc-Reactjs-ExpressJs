...
app.readyHooks.add('Run tool.addSoTienCLC.js', {
    ready: () => app.database.oracle.connected && app.model.tcHocPhiSubDetail,
    run: async () => {
        try {
            console.log('START')
            await app.model.tcHocPhiSubDetail.addTienClc();
            console.log('END');
        } catch (error) {
