...
app.readyHooks.add('Run tool.dongBoHocPhi.js', {
    ready: () => app.database.oracle.connected && app.model.fwStudent && app.model.tcDotDong,
    run: async () => {
        try {
            console.log('START')
            let listSv = await app.model.fwStudent.getAll({}, 'mssv').then(data => data.map(item => item.mssv));
            listSv = Array.from(new Set(listSv)).chunk(50);
            idx = 1
            for (let chunk of listSv) {
                console.log(`CHUNK ${idx}:`);
                await Promise.all(chunk.map(async sv => {
                    await app.model.tcDotDong.capNhatDongTien(sv);
                }))
                idx++;
            }
            console.log('END');
